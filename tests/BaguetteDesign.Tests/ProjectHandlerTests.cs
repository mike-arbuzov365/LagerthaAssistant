...
public sealed class ProjectHandlerTests
{
    ...
    public async Task AddRevision_BelowLimit_IncrementsAndNotifiesClient()
    {
        ...
        // client notification + designer confirmation
        Assert.Equal(2, sender.SentMessages.Count);
        Assert.Equal(999L, sender.SentMessages[0].ChatId); // sent to client
    }
    ...
    public async Task AddRevision_LimitReached_SendsAlert()
    {
        ...
        // client notification + alert to designer
        Assert.Equal(2, sender.SentMessages.Count);
    ...
    [Fact]
    public void Project_IsRevisionLimitReached_WhenAtMax_ReturnsTrue()
    {
    ...
    [Fact]
    public void Project_IsRevisionLimitReached_BelowMax_ReturnsFalse()
    {
    ...
    private sealed class FakeProjectRepository : IProjectRepository
    {
        ...
        public Task<IReadOnlyList<Project>> GetByClientUserIdAsync(string clientUserId, CancellationToken ct = default)
            => Task.FromResult<IReadOnlyList<Project>>(_projects.Where(p => p.ClientUserId == clientUserId).ToList());
    ...
    private sealed class FakeDesignerNotifier : IDesignerNotifier
    {
        ...
        public Task NotifySlotBookedAsync(long clientUserId, Domain.Models.CalendarSlot slot, string? meetLink, CancellationToken ct = default) => Task.CompletedTask;
    }
