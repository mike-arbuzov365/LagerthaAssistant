...
public sealed class InboxHandlerTests
{
    ...
    private sealed class FakeConversationRepository : IConversationRepository
    {
        ...
        public FakeConversationRepository(IReadOnlyList<ConversationSession> sessions) => _sessions = [.. sessions];
        public Task<ConversationSession> FindOrCreateSessionAsync(string userId, CancellationToken ct = default)
        {
            var s = _sessions.FirstOrDefault(x => x.UserId == userId) ?? new ConversationSession { UserId = userId };
            return Task.FromResult(s);
        }
        public Task<ConversationSession?> FindSessionAsync(string userId, CancellationToken ct = default) => Task.FromResult(_sessions.FirstOrDefault(x => x.UserId == userId));
        public Task<IReadOnlyList<ConversationHistoryEntry>> GetRecentHistoryAsync(int sessionId, int limit, CancellationToken ct = default) => Task.FromResult<IReadOnlyList<ConversationHistoryEntry>>([]);
        public Task<IReadOnlyList<ConversationSession>> GetAllClientSessionsAsync(string excludeUserId, CancellationToken ct = default) => Task.FromResult<IReadOnlyList<ConversationSession>>(_sessions.Where(s => s.UserId != excludeUserId).ToList());
    ...
    private sealed class FakeDialogStateRepository : IDialogStateRepository
    {
        ...
        public Task<IReadOnlyList<DialogState>> GetAllAsync(CancellationToken ct = default) => Task.FromResult<IReadOnlyList<DialogState>>([.. _store.Values]);
        public Task<DialogState?> GetByClientUserIdAsync(string clientUserId, CancellationToken ct = default) { _store.TryGetValue(clientUserId, out var val); return Task.FromResult(val); }
        public Task UpsertAsync(DialogState state, CancellationToken ct = default) { _store[state.ClientUserId] = state; return Task.CompletedTask; }
        public Task SaveChangesAsync(CancellationToken ct = default) => Task.CompletedTask;
    }
    ...
    private sealed class FakeRoleRouter : BaguetteDesign.Domain.Interfaces.IRoleRouter
    {
        public UserRole Resolve(long userId) => userId == DesignerUserId ? UserRole.Designer : UserRole.Client;
    }
