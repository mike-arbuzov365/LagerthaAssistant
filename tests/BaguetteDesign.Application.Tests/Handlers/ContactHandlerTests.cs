...
public sealed class ContactHandlerTests
{
    ...
    ) Build(IReadOnlyList<CalendarSlot>? slots = null)
    {
        ...
        var handler = new ContactHandler(calendar, repos, repos, notifier, memory, sender);
    ...
    private sealed class FakeCalendarService : ICalendarService
    {
        ...
        public Task<IReadOnlyList<CalendarSlot>> GetAvailableSlotsAsync(int daysAhead = 7, CancellationToken ct = default) => Task.FromResult(_slots);
        public Task<string?> BookSlotAsync(CalendarSlot slot, string userId, string summary, CancellationToken ct = default) => Task.FromResult<string?>("https://meet.google.com/fake-link");
    }
