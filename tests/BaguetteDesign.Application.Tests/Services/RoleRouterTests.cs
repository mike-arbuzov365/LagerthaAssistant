namespace BaguetteDesign.Application.Tests.Services;
...
public sealed class RoleRouterTests
{
    ...
    [Fact]
    public void Resolve_AdminAndClientAreDistinct_BothWorkCorrectly()
    {
        var router = new RoleRouter(999L);

        Assert.Equal(UserRole.Designer, router.Resolve(999L));
        Assert.Equal(UserRole.Client, router.Resolve(1000L));
    }
}
