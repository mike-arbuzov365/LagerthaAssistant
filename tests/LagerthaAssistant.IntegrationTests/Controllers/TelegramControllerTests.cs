...
[Fact]
    ...
    private sealed class FakeTelegramImportSourceReader : ITelegramImportSourceReader
    {
        ...
        public TelegramInventoryPhotoAnalysisResult NextInventoryPhotoResult { get; set; }
            = new(
                Success: true,
                Candidates:
                [
                    new TelegramInventoryPhotoCandidate(1, "Milk", 2, "pcs", 0.92)
                ],
                Unknown: []);
        ...
        public Task<TelegramInventoryPhotoAnalysisResult> AnalyzeInventoryPhotoAsync(
            string photoFileId,
        ...
        {
            Calls++;
            return Task.FromResult(NextInventoryPhotoResult);
        }
    }
    ...
    private sealed class FakeTelegramNavigationPresenter : ITelegramNavigationPresenter
    {
        ...
        public string GetText(string key, string locale, params object[] args)
        {
            var value = key switch
            {
                ...
                "inventory.reset_stock.prompt" => "Reset stock for all items?",
                ...
                "inventory.photo.empty" => "No confident changes detected",
                ...
                "inventory.photo.expired" => "Photo session expired",
                "inventory.photo.cancelled" => "Photo session cancelled",
        ...
        public TelegramInlineKeyboardMarkup BuildSettingsLaunchKeyboard(string locale)
            => new(
            [
                [new TelegramInlineKeyboardButton("Open settings", Url: "https://t.me/LagerthaAssistantBot?startapp=settings&mode=compact")],
        ...
        public TelegramInlineKeyboardMarkup BuildInventoryPhotoConfirmKeyboard(string locale)
            => new(
            [
                [new TelegramInlineKeyboardButton("Apply all", CallbackDataConstants.Inventory.PhotoApplyAll)],
                [new TelegramInlineKeyboardButton("Select", CallbackDataConstants.Inventory.PhotoSelect), new TelegramInlineKeyboardButton("Cancel", CallbackDataConstants.Inventory.PhotoCancel)]
            ]);
    ...
    public async Task Webhook_ShouldShowGroupedInventoryCatalog_WhenShopAddCallbackRequested()
    {
        var foodService = new FakeFoodTrackingService
        {
            InventoryItems =
            [
                new FoodItemDto(20, "Apple", "Produce", null, null, null) { CurrentQuantity = 2m, IconEmoji = "🍎" },
                new FoodItemDto(70, "Beer", "Beverages", null, null, "6 cans") { IconEmoji = "🍺" },
                new FoodItemDto(85, "Ground pepper", "Condiments", null, null, "1") { IconEmoji = "🌶️" }
            ]
        };
        var sender = new FakeTelegramBotSender();
        var navigationState = new FakeNavigationStateService { CurrentSection = NavigationSections.Shopping };
        ...
        Assert.Contains("Apple [2]", sender.LastText, StringComparison.OrdinalIgnoreCase);
        Assert.Contains("Beer [6 cans]", sender.LastText, StringComparison.OrdinalIgnoreCase);
        Assert.Contains("&#127866;", sender.LastText, StringComparison.Ordinal);
    ...
    public async Task Webhook_ShouldStartInventoryPhotoMode_WhenPhotoRestockCallbackRequested()
    {
        var foodService = new FakeFoodTrackingService
        {
            InventoryItems = [new FoodItemDto(1, "Milk", "Dairy", null, null, null)]
        };
        var sender = new FakeTelegramBotSender();
        var navigationState = new FakeNavigationStateService { CurrentSection = NavigationSections.Inventory };
        var sut = CreateSut(
        ...
        var response = await sut.Webhook(
            BuildCallbackUpdate(1001, 2002, CallbackDataConstants.Inventory.PhotoRestock, null, updateId: 940),
            CancellationToken.None);
    ...
    [Fact]
    public async Task Webhook_ShouldAnalyzeInventoryPhoto_AndApplyAll_WhenRequested()
    {
        var foodService = new FakeFoodTrackingService
        {
        ...
        var importReader = new FakeTelegramImportSourceReader
        {
            NextInventoryPhotoResult = new TelegramInventoryPhotoAnalysisResult(
                ...
                Candidates:
                [
                    new TelegramInventoryPhotoCandidate(1, "Milk", 2m, "pcs", 0.91)
                ],
                Unknown: [])
        };
        var sender = new FakeTelegramBotSender();
        ...
        _ = await sut.Webhook(
            BuildCallbackUpdate(1001, 2002, CallbackDataConstants.Inventory.PhotoRestock, null, updateId: 941),
            CancellationToken.None);

        var previewResponse = await sut.Webhook(
            BuildPhotoUpdate(1001, 2002, "photo-1", null, updateId: 942),
            CancellationToken.None);

        var previewOk = Assert.IsType<OkObjectResult>(previewResponse.Result);
        var previewPayload = Assert.IsType<TelegramWebhookResponse>(previewOk.Value);
        Assert.Equal("inventory.photo.preview", previewPayload.Intent);
        Assert.Contains("Inventory photo suggestions", sender.LastText, StringComparison.OrdinalIgnoreCase);
        Assert.Contains("Milk", sender.LastText, StringComparison.OrdinalIgnoreCase);

        var applyResponse = await sut.Webhook(
            BuildCallbackUpdate(1001, 2002, CallbackDataConstants.Inventory.PhotoApplyAll, null, updateId: 943),
            CancellationToken.None);

        var applyOk = Assert.IsType<OkObjectResult>(applyResponse.Result);
        var applyPayload = Assert.IsType<TelegramWebhookResponse>(applyOk.Value);
        Assert.Equal("inventory.photo.applied", applyPayload.Intent);
    ...
    public async Task Webhook_ShouldRequireInventoryPhotoMode_WhenPhotoSentWithoutPendingMode()
    {
        ...
        var response = await sut.Webhook(
            BuildPhotoUpdate(1001, 2002, "photo-2", null, updateId: 944),
            CancellationToken.None);
    ...
    [Fact]
    public async Task Webhook_ShouldSetAndAdjustMinQuantity_WhenPendingMinAndPastedCatalogProvided()
    {
        var foodService = new FakeFoodTrackingService
        {
            InventoryItems =
            [
                new FoodItemDto(54, "Green peas", "Canned Goods", null, null, null) { MinQuantity = 0m },
                new FoodItemDto(10, "Tomato Sauce", "Canned Goods", null, null, null) { MinQuantity = 1m },
                new FoodItemDto(85, "Ground pepper", "Condiments", null, null, null) { MinQuantity = 1m }
            ]
        };
        var sender = new FakeTelegramBotSender();
        var navigationState = new FakeNavigationStateService { CurrentSection = NavigationSections.Inventory };
        ...
        _ = await sut.Webhook(BuildCallbackUpdate(1001, 2002, CallbackDataConstants.Inventory.Min, null, updateId: 935), CancellationToken.None);
        var response = await sut.Webhook(BuildTextUpdate(1001, 2002, payloadText, null, updateId: 936), CancellationToken.None);

        var ok = Assert.IsType<OkObjectResult>(response.Result);
        var webhookPayload = Assert.IsType<TelegramWebhookResponse>(ok.Value);
        Assert.Equal("inventory.min.done", webhookPayload.Intent);
        Assert.Equal(3, foodService.MinOperations.Count);
        Assert.Contains(foodService.MinOperations, operation => operation == (54, 1m));
        Assert.Contains(foodService.MinOperations, operation => operation == (10, 4m));
        Assert.Contains(foodService.MinOperations, operation => operation == (85, 6m));
    }

    [Fact]
    public async Task Webhook_ShouldResetInventoryStock_WhenResetConfirmed()
    {
        var foodService = new FakeFoodTrackingService
        {
            InventoryItems =
            [
                new FoodItemDto(7, "Beer", "Beverages", null, null, null) { CurrentQuantity = 5m }
            ],
            ResetAllUpdatedCount = 12
        };
        var sender = new FakeTelegramBotSender();
        var navigationState = new FakeNavigationStateService { CurrentSection = NavigationSections.Inventory };
        var sut = CreateSut(
        ...
        var confirmPrompt = await sut.Webhook(
            BuildCallbackUpdate(1001, 2002, CallbackDataConstants.Inventory.ResetStock, null, updateId: 937),
            CancellationToken.None);
        _ = Assert.IsType<OkObjectResult>(confirmPrompt.Result);

        var response = await sut.Webhook(
            BuildCallbackUpdate(1001, 2002, CallbackDataConstants.Inventory.ResetStockConfirm, null, updateId: 938),
            CancellationToken.None);

        var ok = Assert.IsType<OkObjectResult>(response.Result);
        var payload = Assert.IsType<TelegramWebhookResponse>(ok.Value);
        Assert.Equal("inventory.reset_stock.done", payload.Intent);
        Assert.Equal(1, foodService.ResetAllCalls);
        Assert.Contains("12", sender.LastText, StringComparison.Ordinal);
    }
    ...
    private sealed class FakeFoodTrackingService : IFoodTrackingService
    {
        ...
        public int? LastSetCurrentInventoryItemId { get; private set; }
        ...
        public List<(int ItemId, decimal MinQuantity)> MinOperations { get; } = [];
        ...
        public Task<FoodItemDto> AdjustInventoryQuantityAsync(int foodItemId, decimal delta, CancellationToken cancellationToken = default)
        {
            ...
            AdjustOperations.Add((foodItemId, "delta", delta));
        ...
        public Task<FoodItemDto> SetInventoryCurrentQuantityAsync(int foodItemId, decimal quantity, CancellationToken cancellationToken = default)
        {
            ...
            LastSetCurrentInventoryItemId = foodItemId;
            LastSetCurrentQuantity = quantity;
            AdjustOperations.Add((foodItemId, "set", quantity));
        ...
        {
            ResetAllCalls++;
            return Task.FromResult(ResetAllUpdatedCount);
        }
